...
class TestMt940Base < Test::Unit::TestCase
  ...
  context 'MT940::Base' do
    ...
    should 'read the statements with the filename of the MT940 file' do
      file_name = File.dirname(__FILE__) + '/fixtures/ing.txt'
    ...
    should 'read the statements with the handle to the mt940 file itself' do
      file_name = File.dirname(__FILE__) + '/fixtures/ing.txt'
      file = File.open(file_name, 'r:utf-8')
    ...
    should 'read the statements with the handle of a Tempfile' do
      file = Tempfile.new('temp')
      file.write(':940:')
      file.rewind
      @statements = MT940::Base.statements(file)
      assert_equal 0, @statements.size
      file.unlink
    end


    should '@transaction raise an exception if the file does not exist' do
    ...
    should 'statement raise an exception if the file does not exist' do
      file_name = File.dirname(__FILE__) + '/fixtures/123.txt'
      assert_raise Errno::ENOENT do
        @transactions = MT940::Base.statements(file_name)
      end
    end
  ...
  context 'Unknown MT940 file' do
    should 'transaction return its bank' do
      file_name = File.dirname(__FILE__) + '/fixtures/unknown.txt'
      @transactions = MT940::Base.transactions(file_name)
      assert_equal 'Unknown', @transactions.first.bank
    end

    should 'statement return its bank' do
      file_name = File.dirname(__FILE__) + '/fixtures/unknown.txt'
      @statements = MT940::Base.statements(file_name)
      assert_equal 'Unknown', @statements.first.bank
    end
